...
public class RazorModel
{
    ...
    public List<DbColumnInfo> columns => this.table.Columns.OrderBy(a => a.Position).ToList();
    public string NameSpace { get; set; }
    public string FullTableName => $"{(new[] { "public", "dbo" }.Contains(table.Schema) ? "" : table.Schema)}.{table.Name}".TrimStart('.');
    public string TableName => table.Name;
    public string CommonModule => "Common";
    public string ModuleName => string.IsNullOrEmpty(GetModuleName()) ? "" : "." + GetModuleName();
    public string TableComment => GetTableComment();
    public List<string> ExtendsOneList => new List<string>
    {
        "Id","CreationTime", "CreatorUserId", "CreatorUserName", "LastModificationTime", "LastModifierUserId", "LastModifierUserName", "IsDeleted"
    };

    public List<string> ExtendsTwoList => new List<string>
    {
        "Id","CreationTime", "CreatorUserId", "CreatorUserName", "LastModificationTime", "LastModifierUserId", "LastModifierUserName"
    };

    public List<string> ExtendsThreeList => new List<string>
    {
        "Id","IsDeleted"
    };

    public List<string> ExtendsFourList => new List<string>
    {
        "Id"
    };

    public ExtentsType ExtentsType => GetExtentsType();

    public List<string> NullableTypes => new List<string> {
        "int", "long", "bool", "DateTime", "decimal", "float","double"
    };

    public string GetFirstRowCsName(string name)
    {
        var text = GetCsName(name);
        if (text.Length <= 1) return text.ToLower();
        else return text.Substring(0, 1).ToLower() + text.Substring(1, text.Length - 1);
    }

    /// <summary>
    /// 获取模块名称（未按规范补充表注释则统一放到Common）
    /// </summary>
    /// <returns></returns>
    public string GetModuleName()
    {
        if (string.IsNullOrWhiteSpace(table.Comment)) return CommonModule;
        var strs = table.Comment.Split("-");
        if (strs.Count() >= 2)
        {
            return GetCsName(strs[0]);
        }
        else
        {
            return CommonModule;
    ...
    public string GetTableComment()
    {
        ...
        if (table.Comment.Contains('-'))
        {
            ...
            if (strs.Count() >= 2)
            {
                return table.Comment.Replace(strs[0] + '-', "");
            }
        }
    ...
    public string GetQueryCsType(DbColumnInfo col)
    {
        var type = GetCsType(col);
    ...
    public ExtentsType GetExtentsType()
    {
        var names = columns.Select(a => GetCsName(a.Name)).ToList();
        if (ExtendsOneList.All(a => names.Any(x => x == a)))
        {
            return ExtentsType.One;
        }
        else if (ExtendsTwoList.All(a => names.Any(x => x == a)))
        {
            return ExtentsType.Two;
        }
        else if (ExtendsThreeList.All(a => names.Any(x => x == a)))
        {
            return ExtentsType.Three;
        }
        else if (ExtendsFourList.All(a => names.Any(x => x == a)))
        {
            return ExtentsType.Four;
        }

        return ExtentsType.Unknow;
    }
    ...
    public string GetExtends()
    {
        ...
        switch (ExtentsType)
        {
            case ExtentsType.One:
                result = $": EntityBaseWithSoftDelete<{GetPrimaryTypeStr()}>";
                break;
            case ExtentsType.Two:
                result = $": EntityBase<{GetPrimaryTypeStr()}>";
                break;
            case ExtentsType.Three:
                result = $": EntityWithSoftDelete<{GetPrimaryTypeStr()}>";
                break;
            case ExtentsType.Four:
                result = $": Entity<{GetPrimaryTypeStr()}>";
                break;
        }
        return result;
    }

    public string GetPrimaryTypeStr()
    {
        var col = columns.Where(a => a.IsPrimary).FirstOrDefault();
        if (col == null)
        {
            return "object";
        }
